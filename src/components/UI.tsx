...
import {
  ChevronRight, Sun, Moon, Cpu, Award, Play, Trophy, AlertTriangle,
  Lightbulb, RefreshCw, Send, ArrowLeft, ExternalLink, Terminal as TerminalIcon,
...
export const DialogueBox: React.FC<{
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 40 }} className="vn-dialogue">
      <div className="relative h-full flex gap-0">
        {/* Portrait */}
        <div className="absolute -bottom-2 left-4 w-60 md:w-80 pointer-events-none z-20 flex items-end" style={{ height: '360px' }}>
          <motion.div className="w-full h-full" initial={{ x: -60, opacity: 0 }} animate={{ x: 0, opacity: 1 }}>
            <div className="absolute -inset-10 bg-dg-pink/5 blur-3xl rounded-full" />
            <CharacterSprite mentor={mentor} isTalking={isTalking} className="w-full h-full" />
          </motion.div>
        </div>

        {/* Text content */}
        <div className="flex-1 pl-64 md:pl-76 pt-5 pr-6 pb-4 flex flex-col justify-between">
          <div>
          ...
          {!isTalking && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} onClick={onComplete}
...
export const StatsBar: React.FC = () => {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-50 h-16 flex items-center px-5 md:px-8 gap-5 border-b border-white/5 backdrop-blur-xl bg-black/70">
      {/* Level diamond */}
      <div className="relative h-12 w-12 flex items-center justify-center shrink-0">
        ...
        <span className="relative z-10 font-bold text-xl text-white font-display">{userData.level}</span>
      </div>
      {/* User info */}
      ...
      <div className="flex-1 hidden md:flex flex-col gap-1 max-w-sm">
        ...
        <div className="h-3 bg-white/5 border border-white/8 overflow-hidden">
      ...
      {userData.streak > 1 && (
        <div className="hidden sm:flex items-center gap-1.5 text-orange-400 text-xs font-bold shrink-0">
          <span className="text-base">🔥</span> {userData.streak}
        </div>
      )}
      {/* Controls */}
      <div className="flex items-center gap-3 ml-auto">
        <button onClick={() => setLanguage(language === 'en' ? 'fr' : 'en')}
          className="h-9 px-3 border border-white/10 hover:border-dg-pink transition-all text-[10px] font-bold uppercase tracking-widest text-white/40 hover:text-white cursor-pointer font-mono">
          {language.toUpperCase()}
        </button>
        <button onClick={toggleTheme} className="h-9 w-9 flex items-center justify-center border border-white/10 hover:border-dg-pink transition-all text-white/40 hover:text-white cursor-pointer">
...
export const XPToast: React.FC<{ xp: number; visible: boolean }> = ({ xp, visible }) => (
  <AnimatePresence>
    {visible && (
      <motion.div initial={{ opacity: 0, y: 40, scale: 0.8 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -20, scale: 0.9 }}
        className="fixed bottom-32 left-1/2 -translate-x-1/2 z-[200] pointer-events-none">
        <div className="bg-dg-pink text-white font-display font-bold text-2xl px-8 py-3 transform -skew-x-6 shadow-[0_0_40px_rgba(255,51,153,0.6)]">
          +{xp} XP
        </div>
      </motion.div>
    )}
